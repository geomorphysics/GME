...
# SymPy
from sympy import Eq, Abs, lambdify, Rational, Matrix, poly, Poly, factor, tan, \
...
class SlicingMath:
    ...
    def __init__(self,
        ...
        self.pxhat_lambda: Callable = lambda sub_, rxhat_, pzhat_: solve(
        ...
        self.pzhat_lambda: Callable = lambda sub_, rxhat_, pxhat_: (solve(
            simplify(self.H_Ci_eqn.subs({rxhat:rxhat_, H:Rational(1,2), mu:eta/2})
    ...
    def define_H_lambda(self,
        ...
        """
        self.H_lambda: Callable =  lambdify(var_list, self.H_Ci_eqn.rhs
    ...
    def define_detHessianSqrd_lambda(self,
        ...
        """
        self.detHessianSqrd_lambda: Callable =  lambdify(var_list,(
    ...
    def define_Ci_lambda(self,
        ...
        """
        self.Ci_lambda: Callable = lambdify(var_list,
    ...
    def define_gstarhat_lambda(self,
        ...
        """
        self.gstarhat_lambda: Callable =  lambdify( var_list,
    ...
    def define_v_pxpzhat_lambda(self,
        ...
        Obtained using: :math:`\mathbf{\hat{v}} = \mathbf{\hat{g}}_* \mathbf{\hat{p}}`.
        """
        self.v_pxpzhat_lambda: Callable = lambdify( (pxhat,pzhat),
    ...
    def define_modv_pxpzhat_lambda(self,
        ...
        Obtained using: :math:`\mathbf{\hat{v}} = \mathbf{\hat{g}}_* \mathbf{\hat{p}}`.
        """
        self.modv_pxpzhat_lambda: Callable = lambdify( (pxhat,pzhat),
            simplify(((self.gstarhat_eqn.rhs.subs({mu:eta/2}).subs(sub_))
...
class SlicingPlots(Graphing):
    ...
    def plot_modv_slice(self,
        ...
        pxhat_eqn_: Eq = sm.pxhatsqrd_Ci_polylike_eqn(sub_, pzhat) #.subs(sub_)
        ...
        modv_array: np.ndarray = np.array([sm.modv_pxpzhat_lambda(pxhat_,pzhat_)
                                    for pxhat_,pzhat_ in zip(pxhat_array,pzhat_array)])
        projv_array: np.ndarray = modv_array*np.cos(np.arctan(-pxhat_array/pzhat_array))
