...
class Manuscript(Graphing):
    ...
    def point_pairing(
        ...
        """
        Schematic illustrating how points pair between successive
        erosion surfaces.
        ...
        def isochrones_subfig(
            ...
            motion_angle_ = 23
            my_arrow_style = ArrowStyle.Simple(head_length=1*sf1_,
                                               head_width=1*sf1_,
                                               tail_width=0.1*sf1_)
            axes_0.annotate('',
                            xy=arrow_xy_,
        ...
        def zoom_boxes(
            fig_: Figure,
            ta_color_: str = gray2_,
            tb_color_: str = gray1_
            ...
            def zoomed_isochrones(
                ...
                if not do_pts_only:
                    plt.text(*(xy_pt2+np.array([-0.03, 0.08])),
                             r'$\mathbf{a}$',
                             color=ta_color_,
                             fontsize=18,
                             rotation=0,
                ...
                if not do_pts_only:
                    ...
                    b_label_text = r'$\{\mathbf{b}\}$' if do_many \
                        else r'$\mathbf{b}$'
                    plt.text(*b_label_xy,
                             b_label_text,
                             color=tb_color_,
            ...
            def v_arrow(
                ...
                v_lw = 1.5
                axes_.arrow(*((xy_pt1*a_f+xy_pt2*(1-a_f))),
                            *((xy_pt1-xy_pt2)*0.01),
                            lw=1,
                            facecolor=color_,
                            edgecolor=color_,
            ...
            def beta_label(
                ...
                axes_.plot([xy_pt2_B[0], xy_pt2_B[0]],
            ...
            def alpha_label(
                ...
                axes_.add_patch(Arc(xy_pt2_B,
                                    ...
                                    theta1=0,
                                    theta2=-angle_B))
                axes_.plot([xy_pt2_B[0], xy_pt2_B[0]+0.5],
            ...
            (xy_pts1_A, xy_pt2_A, _) \
                = zoomed_isochrones(axes_A,
                                    'free',
                                    i_pts1_A,
                                    i_pt2_A,
                                    do_many=True)
            ...
            i_pt2_B = i_pt2_A
            i_pt1_B = i_pt2_A+19
            (xy_pt1_B, xy_pt2_B, dxy_B) \
                = zoomed_isochrones(axes_B,
                                    'isotropic',
                                    i_pt1_B,
                                    i_pt2_B)
            ...
            i_pt1_C = i_pt1_B+30
            i_pt2_C = i_pt2_B
            (xy_pt1_C, xy_pt2_C, dxy_C) \
                = zoomed_isochrones(axes_C,
                                    'anisotropic',
                                    i_pt1_C,
                                    i_pt2_C,
                                    do_legend=True)
            ...
            v_arrow(axes_C,
                    ...
                    dxy=(0.1, 0.05),
                    do_label=True)
            _ = zoomed_isochrones(axes_C,
                                  '',
                                  i_pt1_C,
                                  i_pt2_C,
                                  do_legend=False,
                                  do_pts_only=True)
            psi_label(axes_C, xy_pt1_B, xy_pt2_B,
                      xy_pt1_C, xy_pt2_C, color_=purple_)
    ...
    def covector_isochrones(
        ...
        plt.plot(px_array,
                 ...
                 color=p_color,
                 solid_capstyle='butt')
        plt.plot((px_array[0], px_array[0]),
                 pz_array,
                 lw=lw_pxz_,
                 alpha=alpha_p_,
                 color=p_color,
        ...
        x_off, y_off = 0, -0.4
        my_arrow_style \
            = mpatches.ArrowStyle.Fancy(head_length=.99,
                                        head_width=.6,
                                        tail_width=0.01)
        axes.annotate('',
                      xy=(x_limit/10-0.05+x_off,
                          y_limit*0.7+y_off),
                      xytext=(x_limit/10-0.275+x_off,
                              y_limit*0.7+y_off),
                      arrowprops=dict(arrowstyle=my_arrow_style,
                                      color='k'))
        axes.annotate('',
                      xy=(x_limit/10-0.265+x_off,
                          y_limit*0.7+0.2+y_off),
                      xytext=(x_limit/10-0.265+x_off,
                              y_limit*0.7-0.01+y_off),
                      arrowprops=dict(arrowstyle=my_arrow_style,
                                      color='k'))
        plt.text(x_limit/10-0.05+x_off,
                 y_limit*0.7+y_off,
                 '$x$',
                 ...
                 fontsize=20,
                 color='k')
        plt.text(x_limit/10-0.265+x_off,
                 y_limit*0.7+0.2+y_off,
                 '$z$',
                 horizontalalignment='center',
                 verticalalignment='bottom',
        ...
        # Angle arc and text
        arc_radius = 1.35
        axes.add_patch(mpatches.Arc((0, 0),
                                    arc_radius,
                                    arc_radius,
                                    color='Gray',
                                    linewidth=1.5,
        ...
        plt.text(origin_[0]+off_+0.02+off_x,
        ...
        plt.arrow(*origin_,
                  ...
                  fc=u_color,
                  overhang=0.1)
        plt.arrow(origin_[0]+u_perp*np.tan(beta_)*af,
                  origin_[1]-u_perp*af,
        ...
        plt.arrow(*origin_,
                  ...
                  fc='w',
                  overhang=0.1)
        plt.arrow(origin_[0]+u_horiz*af,
                  origin_[1],
                  u_horiz*naf-0.015,
                  0,
                  head_width=0.0,
                  head_length=0,
                  length_includes_head=True,
                  lw=lw_u, alpha=alpha_,
                  ec=u_color,
                  fc='w',
                  overhang=0.1)

        # u text annotations
        plt.text(origin_[0]+0.23,
                 origin_[1]+0.09,
                 r'${\xi}^{\!\rightarrow} $'
    ...
    def huygens_wavelets(
        ...
        r"""
        Plot the loci of :math:`\mathbf{\widetilde{p}}`
        and :math:`\mathbf{r}` and their behavior defined by :math:`F`
        relative to the :math:`\xi` circle.
        ...
        def trace_indicatrix(
            ...
            pz_array = np.linspace(np.log2(pz_max_/pz_min_),
                                   np.log2(pz_min_/pz_min_),
                                   n_points,
                                   endpoint=True)
            fgtx_pz_array = -pz_min_*np.power(2, pz_array)
            idtx_rdotx_array \
                = np.array([float(rdotx_pz_lambda(pz_))
                            for pz_ in fgtx_pz_array])
            idtx_rdotz_array \
                = np.array([float(rdotz_pz_lambda(pz_))
                            for pz_ in fgtx_pz_array])
            return (idtx_rdotx_array*sf+xy_offset[0],
        ...
        # New isochrones
        plt.plot(gmes.h_x_array+dx_fudge,
                 gmes.h_z_array + dz_,
        ...
        i_list = [111] if do_ray_conjugacy else \
            [int(i) for i in i_start+(i_end-i_start)
             * np.linspace(0, 1, n_i)**0.7]
        ...
        for idx, i_ in enumerate(i_list):
            ...
            (drx_, drz) \
                = gmes.rdotx_interp(rx_)*dt_, gmes.rdotz_interp(rx_)*dt_
            ...
            (idtx_rdotx_array, idtx_rdotz_array) \
                = trace_indicatrix({varphi: varphi_},
                                   n_points=n_points,
                                   xy_offset=(rx_, rz_),
                                   sf=dt_,
                                   pz_min_=1e-3,
            ...
            if not do_ray_conjugacy:
                k_ = 0
                plt.plot(
                    [rx_*k_+idtx_rdotx_array[0]*(1-k_), idtx_rdotx_array[0]],
                ...
                plt.plot([rx_, idtx_rdotx_array[0]],
