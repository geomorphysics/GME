...
class IndicatrixNew(Graphing):
    ...
    def __init__(
                ...
                = Eq(pz_min, (
                    solve(Eq(((
                        solve(Eq(4*gmeq.H_eqn.rhs**2, 1)
                            ...
                            ).args[0].args[0].args[0])**2, 0), pz**4)[0]
                        )**Rational(1, 4)
                     )
            px_min_eqn \
                = Eq(px_min,
        ...
        self.p_infc_array = p_array[np.abs(
            p_array[:, 0]/p_array[:, 1]) < tanbeta_crit]
        self.p_supc_array = p_array[np.abs(
            p_array[:, 0]/p_array[:, 1]) >= tanbeta_crit]

        v_from_gstar_lambda_tmp \
            = lambdify((px, pz),
        ...
        def v_lambda(pa): return np.array([(self.v_from_gstar_lambda(px_, pz_))
                                           for px_, pz_ in pa])
    ...
    def Fstar_F_rectlinear(
        ...
        """
        name: str \
            = f'{job_name}_Fstar_F_rectlinear{"_zoom" if do_zoom else ""}'
        ...
        pz_abmax_ = pz_max_
        vx_abmax_, vz_abmax_ \
            = self.v_from_gstar_lambda(px_abmax_, pz_abmax_)
        px_abcrit_ = -pz_max_*gmeq.tanbeta_crit
        pz_abcrit_ = pz_max_
        vx_abcrit_, vz_abcrit_ \
            = self.v_from_gstar_lambda(px_abcrit_, pz_abcrit_)

        # Lines visualizing critical, bounding angles: ray velocity
        if eta_ == Rational(3, 2):
            plt.plot([0, vx_abmax_],
                     [0, vz_abmax_],
                     '-', color='r',
                     alpha=0.4,
                     lw=2,
        ...
        pz_ = - float(solve(self.H_parametric_eqn.subs(
                    {px: pz*(gmeq.tanbeta_crit)}), pz)[0])
    ...
    def Fstar_F_polar(
        ...
        if eta_ > 1:
            ...
            def alpha_fn(a): return np.pi-a
        else:
            r_min_, r_max_ = 0.1, 10
            def alpha_fn(a): return a

        def v_scale_fn(v): return scale_fn(v)*1

        # Lines visualizing critical, bounding angles: ray velocity
        if eta_ > 1:
            plt.polar([np.pi/2+(np.arctan(gmeq.tanalpha_ext))]*2,
                      ...
                      lw=2,
                      label=r'$\alpha_{\mathrm{lim}}$')
        plt.polar(alpha_fn(np.arcsin(self.v_supc_array[:, 0]
                                     / norm(self.v_supc_array,
                                            axis=1))),
        ...
        plt.polar([np.pi/2+(np.arctan(gmeq.tanalpha_ext))]*2,
                  ...
                  lw=1,
                  label=r'$\alpha_{\mathrm{c}}$')
        plt.polar(alpha_fn(np.arcsin(self.v_infc_array[:, 0]
                                     / norm(self.v_infc_array,
                                            axis=1))),
        ...
        if eta_ > 1:
            plt.polar([np.arctan(self.tanbeta_max)]*2,
                      [scale_fn(r_min_),
                       scale_fn(r_max_)],
                      '-', color='b',
                      alpha=0.3,
                      ...
                      label=r'$\beta_{\mathrm{max}}$')
        plt.polar(
            np.arcsin(self.p_supc_array[:, 0]/norm(self.p_supc_array, axis=1)),
            scale_fn(norm(self.p_supc_array, axis=1)),
            'b' if eta_ > 1 else 'DarkBlue',
            label=r'$F^*\!\!=1$,  $\beta\geq\beta_\mathrm{c}$'
        )
        plt.polar([np.arctan(gmeq.tanbeta_crit)]*2,
                  [scale_fn(r_min_), scale_fn(r_max_)],
                  '--', color='DarkBlue' if eta_ > 1 else 'b',
                  lw=1,
                  label=r'$\beta_{\mathrm{c}}$')
        plt.polar(
            np.arcsin(self.p_infc_array[:, 0]/norm(self.p_infc_array, axis=1)),
        ...
        if eta_ > 1:
            ...
            axes.text(np.deg2rad(85+theta_max_),
                      0.5,
                      vert_label,
                      rotation=theta_max_,
                      ha='center',
            ...
            theta_max_ = 0
            axes.set_thetamax(90+theta_max_)
            axes.text(np.deg2rad(92+theta_max_),
                      axes.get_rmax()/5,
                      vert_label,
                      rotation=theta_max_,
                      ha='right',
        ...
        plt.polar(alpha_fn(
                        (np.arcsin(self.v_supc_array[-1, 0]
                                   / norm(self.v_supc_array[-1]))
                         + np.arcsin(self.v_infc_array[0, 0]
                                     / norm(self.v_infc_array[0])))/2),
