...
techniques. This notebook demonstrates this generality.
    ...
    To get from an erosion equation to a geomorphic Hamiltonian, we need to
make the following conceptual leaps:
...
\item
  ...
  \tightlist
  \item
    so the erosion equation should be written as an \emph{erosion speed
    in the surface-normal direction};
  \end{itemize}
\item
  accept that the rate of motion of a surface is correctly expressed as
  a covector, aka a covariant rank-1 tensor, rather than as a vector,
  aka a contravariant rank-1 tensor

  \begin{itemize}
  \tightlist
  \item
    so the erosion equation should in fact be written as an
    \emph{erosion slowness in the surface-normal direction};
  \end{itemize}
\item
...
where \(\mathcal{F}_*=1\), obviously. So we have the eikonal equation
\begin{equation}
...
where we know that \(\mathcal{F}_*=1\). The generic 2D geomorphic
Hamiltonian is then defined in terms of \(\mathcal{F}_*\) as
\begin{equation}
    \mathcal{H}(\mathbf{p},\mathbf{r})
